...
import { clusterStore, projectStore } from '../../stores';
...
function BaseLayout(): JSX.Element {
  ...
  const [, setUrlPrefix] = useStore<string>('urlPrefix');
  ...
  const [projectResult, clusterResult, workspaceRuleResult] = useQueries([
    ...
    },
    {
      ...
      queryFn: () => fetchRules({ workspace, name: userName }),
      enabled: !!workspace,
    },
  ]);
  const holeRules = useQuery(
    ['holeRules'],
    () => fetchRules({ cluster, workspace, namespace, name: userName }),
    {
