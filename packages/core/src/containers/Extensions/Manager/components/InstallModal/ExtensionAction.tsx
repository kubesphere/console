...
function ExtensionAction({
  ...
  const { isFetching: isExtensionVersionFilesQueryFetching } = useExtensionVersionFilesQuery({
    enabled: !!selectedVersion,
    extensionName,
    version: selectedVersion ?? '',
    onSuccess: data => {
      const configFile = find(
        data.formattedExtensionVersionFiles,
        ({ name }) => name.toLowerCase() === 'values.yaml',
      );
      const versionConfig = configFile?.data;
      let configYaml = '';
      if (isExtensionUpdate) {
        configYaml = formattedInstallPlan?.config ?? versionConfig ?? '';
      } else {
