...
export default {
  ...
  DEVOPS_DESCRIPTION:
    'DevOps 是一個獨立的命名空間，其中定義了一組流水線；使用者可以按照自己的方式對流水線進行分組 (例如：項目類型、組織類型)。',
  DEVOPS_PROJECT_CREATE_DESC:
    'DevOps 是一個獨立的命名空間，其中定義了一組的流水線；使用者可以按照自己的方式對流水線進行分組 (例如：項目類型、組織類型)。',
  ...
  DEVOPS_PROJECT_ROLES_DESC:
    '成員角色定義了在目前 DevOps 工程下使用者所擁有的權限。',

  DEVOPS_PROJECT_MEM_DESC: '對工程内的成員進行管理及角色分配',

  DELETE_DEVOPS_TIP:
    '確定刪除 DevOps 工程 <strong>{resource}</strong> ? 刪除後將無法恢復, 工程下的資源也同時會被銷毁。',

  DEVOPS_TIP_GITOPS_Q: '如何落地 GitOps？',
  DEVOPS_TIP_GITOPS_A:
    '創建關聯代碼倉庫的流水線，並積極使用部署 Kubernetes 部署步驟，就可以實現 GitOps 了。',

  DEVOPS_TIP_TYPE_Q: '流水線 (Pipeline) 支持哪些代碼倉庫類？',
  DEVOPS_TIP_TYPE_A:
