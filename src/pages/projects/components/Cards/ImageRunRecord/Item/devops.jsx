...
export default class ImageBuilderLastRun extends React.Component {
  ...
  renderLog = () => {
    const { data: log, isLoading } = this.store.list
  ...
  render() {
    const { status, imageName, startTime, name } = this.props.runDetail
    ...
    return (
      ...
      >
        <li>
          <div className={styles.value}>{this.props.runDetail?.name}</div>
        ...
        <li>
          <div className={styles.value}>
            <Status
              ...
              type={status}
              name={t(`RUN_${status.toUpperCase()}`)}
              flicker
            />
          </div>
