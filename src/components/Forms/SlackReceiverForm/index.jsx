...
export default class SlackReceiverForm extends Component {
  render() {
    return (
      <Form
    ...
    if (this.props.showTip && this.props.formStatus === 'update') {
      return (
        <Alert
          ...
          type="error"
          message={t('SLACK_CHANNEL_SETTINGS_CHANGE_NEED_SAVE_TIP')}
        />
      )
    }
    return null
  ...
  renderFormItems() {
    return (
      <div className={styles.row}>
        <Form.Item
          label={t('Channel')}
          rules={[{ required: true, message: t('Please input channel') }]}
        >
          <Input name="spec.channel" />
        </Form.Item>
      </div>
    )
  }
