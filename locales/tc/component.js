...
module.exports = {
  ...
  'KS-APISERVER_DESC': '整個集群管理的 API 介面和集群内部各個模組之間通信的樞紐，以及集群安全控制',
  ...
  'KUBE-CONTROLLER-MANAGER_DESC': '由一系列的控制器組成，處理集群中常規任務的後台線程',
  'KUBE-SCHEDULER_DESC': 'Kubernetes 的調度器，將 Pod 調度到合適的 Node 節點上去',
  'METRICS-SERVER_DESC': 'Kubernetes 的監控組件，從每個節點的 Kubelet 採集指標資訊',
  'TILLER-DEPLOY_DESC': 'Helm 的服務端，負責管理發佈 release',
  'OPENPITRIX-API-GATEWAY_DESC': '負責處理平台的服務請求和處理 API 調用過程中的所有任務',
  ...
  'OPENPITRIX-IAM-SERVICE_DESC': '控制哪些用戶可使用您的資源（身份驗證）以及可使用的資源和採用的方式（授權）',
  ...
  'OPENPITRIX-TASK-MANAGER_DESC': '具體執行 OpenPitrix 應用實例生命週期 Action 子任務',
  'ISTIO-CITADEL_DESC': '透過内建身份和憑證管理賦予强大的服務之間和最終用戶身份驗證',
  'ISTIO-GALLEY_DESC': '代表其他的 Istio 控制平面組件，用來驗證用戶編寫的 Istio API 配置',
  'ISTIO-INGRESSGATEWAY_DESC': '提供外網訪問的網關',
  'ISTIO-PILOT_DESC': '為 Envoy sidecar 提供服務發現功能',
  'ISTIO-POLICY_DESC': '用於向 Envoy 提供訪問策略控制，黑白名單控制，速率限制等相關策略',
  'ISTIO-SIDECAR-INJECTOR_DESC': '為配置注入的 Pod 自動注入 sidecar',
  'ISTIO-TELEMETRY_DESC': '為 Envoy 提供了數據上報和紀錄搜集服務',
  'JAEGER-COLLECTOR_DESC': '收集 sidecar 的數據，istio 裡面 sidecar 就是 jaeger-agent',
  'JAEGER-COLLECTOR-HEADLESS_DESC': '收集 sidecar 的數據，Istio 裡面 sidecar 就是 jaeger-agent',
  'JAEGER-OPERATOR_DESC': '負責創建 jaeger 服務，並在配置更新時自動應用到 Jaeger 服務',
  'JAEGER-QUERY_DESC': '接收查詢請求，然後從後端儲存系統中檢索 trace 並透過 UI 進行展示',
  'KUBE-STATE-METRICS_DESC': '監聽 Kubernetes API server 以獲取集群中各種 API 對象的狀態包括節點，工作負載和 Pod 等，並生成相關監控數據供 Prometheus 抓取',
  'NODE-EXPORTER_DESC': '收集集群各個節點的監控數據，供 Prometheus 抓取',
  'PROMETHEUS-K8S_DESC': '提供節點、工作負載、 API 對象相關監控數據',
  'PROMETHEUS-K8S-SYSTEM_DESC': '提供 etcd, coredns, kube-apiserver, kube-scheduler, kube-controller-manager 等 Kubernetes 組件的監控數據',
  'PROMETHEUS-OPERATED_DESC': '所有 Prometheus 實例對應的服務，供 Prometheus Operator 内部使用',
  'PROMETHEUS-OPERATOR_DESC': '管理 Prometheus 實例的 Operator',
  'ELASTICSEARCH-LOGGING-DATA_DESC': '提供 Elasticsearch 數據儲存、備份、搜索等數據服務',
  ...
  'UC-JENKINS-UPDATE-CENTER_DESC': 'Jenkins 更新中心，提供 Jenkins 插件的安裝包',
  ...
  'KUBE-SCHEDULER-SVC_DESC': 'Kubernetes 的調度器，將 Pod 調度到合適的 Node 節點上去',
  'KUBE-CONTROLLER-MANAGER-SVC_DESC': '由一系列的控制器組成，處理集群中常規任務的背景執行緒',
  'S2IOPERATOR-METRICS-SERVICE_DESC': 'S2I 監控服務組件，提供基礎監控數據',
  MINIO_DESC: '一個高性能的開源對象儲存伺服器，適合儲存大容量非結構化的數據',
  'ALERTMANAGER-OPERATED_DESC': 'Alertmanager 服務，用於 Prometheus 等與 Alertmanager 集成',
  'ALERTMANAGER-MAIN_DESC': 'Alertmanager Web UI 服務',
  'NOTIFICATION-MANAGER-SVC_DESC': 'Notification Manager 服務，提供發送郵件、微信、Slack 等通知的接口',
  'NOTIFICATION-MANAGER-CONTROLLER-METRICS_DESC': '提供 Notification Manager Controller 内部監控數據的服務',
  HYPERPITRIX_DESC: '針對基於 Helm 的應用程式提供應用商店服務，管理應用生命週期',
  'JAEGER-OPERATOR-METRICS_DESC': '提供 operator 的監控 metrics',
  'LOGSIDECAR-INJECTOR-ADMISSION_DESC': '為指定 Pod 自動注入落盤紀錄收集 Sidecar 容器',
  'KS-EVENTS-ADMISSION_DESC': '為 Events 規則管理提供驗證 webhook',
  'KS-EVENTS-RULER_DESC': 'Events 規則引擎服務，提供 Events 過濾和告警功能',
  'KS-CONTROLLER-MANAGER_DESC': '實現業務邏輯，例如創建企業空間時，為其創建對應的權限；創建服務策略時，生成對應的 Istio 配置等',
