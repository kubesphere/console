...
module.exports = {
  ...
  'Add Step': '添加步驟',
  'All branches': '所有分支',
  'Alternate options, one per line. The first line will be used as the default option.': '備用選擇，每行一個。第一行的將作為預設選項。',
  'Any repository containing Jenkinsfile will be built automatically.': '任何包含 Jenkinsfile 的倉庫將會自動構建',
  archiveArtifacts: '保存成品',
  'Are you sure to close this pipeline Editor ?': '確定關閉流水線編輯？',
  'Are you sure to close this jenkinsfile Editor ?': '確定關閉 jenkinsfile 編輯？',
  artifacts: '成品',
  Artifacts: '成品',
  ...
  'Executes the code inside the block with a determined time out limit.': '使用確定的超時限制執行塊内的代碼.',
  expression: '表達式',
  Failure: '失敗',
  ...
  'If you check this option, you cannot run multiple builds concurrently.': '如果勾選此選項，則不能同時運行多個 build.',
  info: '提示',
  input: '審核',
  ...
  Issues: '問題',
  'It specifies the default value of the field, allowing the user to save the typed actual value.': '指定字段的預設值，允許用戶保存鍵入實際值。',
  ...
  'Load the sonarqube configuration provided by Jenkins into the Pipeline.': '將 Jenkins 中的 sonarqube 配置加載到流水線中',
  mail: '郵件',
  major: '重要',
  ...
  'Old builds will be deleted after this number of days.': '如果構建達到保留的天數將進行刪除',
  'Old builds will be discarded after the build number exceeds the maximum amount.': '如果構建超過一定的數量將丢棄舊的構建',
  ...
  'Please select a code repository as the code source for the pipeline.': '請選擇一個代碼倉庫作為 Pipeline 的代碼來源',
  ...
  'Specify a container to add nested tasks to execute inside the container': '指定容器，可添加嵌套任務在容器内執行',
  'Specify the location of the Jenkinsfile in the source code repository': '指定 Jenkinsfile 在源代碼倉庫的位置',
  ...
  'Test Result': '檢測結果',
  'Text Variable': '文本變量',
  'The conditions required to implement the current phase (optional).': '執行目前階段段所需要滿足的條件 （可選）',
  'The default value here is the current user\'s kubeconfig.': '這裡的預設值為目前用户的 kubeconfig',
  'The environment variable entered before running the pipeline is match the current value.': '運行流水線前輸入的環境變量與目前值一致',
  'The label on which to run the Pipeline or individual stage': '流水線或單個階段的標籤',
  'The name of the pipeline. Pipelines in the same project must have different names.': 'Pipeline 的名稱，同一個項目内 Pipeline 不能重名',
  ...
  'Trigger a remote build (for example, using a script)': '觸發遠程構建 (例如，使用腳本)',
  'Trusted User': '可信任用戶',
  Unnamed: '未命名',
  ...
  'username or group name, multiple values ​​used, separated': '用戶名或組名，多個值使用英文逗號分隔',
  ...
  'You can execute shell commands or windows batch commands in the build.': '可以在構建中執行 shell 命令或者 windows 的 batch 命令',
  ...
  ADD_NEW_CREDENTIAL_DESC: '獲取倉庫代碼可能需要憑證, 選擇已有憑證或添加一個新的憑證',
  ...
  VULNERABILITY: '漏洞',
  SECURITY_HOTSPOT: '安全',
  REG_FILTER_DESC: '啟用正則表達式，將忽略與提供的正則表達式不匹配的名稱（包括分支與PR等)',
  waitForQualityGate_desc: '代碼質量檢查標準來源於SonarQube的 Quality Gate (質量閥)，如果需要自定義檢查標準請前往 SonarQube 設置',
  ...
  PIPELINE_QUEUED_DESC: '您需要等待 agent 啟動並執行流水線（注：如 agent 長時間沒有啟動請檢查 agent 配置和集群資源情況）',
  pipeline_owner: 'DevOps 工程的所有者，可以進行 DevOps 工程的所有操作',
  pipeline_maintainer: 'DevOps 工程的主要維護者，可以進行項目内的憑證配置、Pipeline 配置等操作',
  ...
  WHEN_CREATE_PIPELINE_DESC: '當有創建新的流水線時，將自動觸發指定流水線中的任務。',
