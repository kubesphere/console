...
module.exports = {
  ...
  'Job Settings': '任務設置',
  'Job Template': '任務模板',
  'KubeSphere supports pulling images from the Image Registries and creating new images through source code (Source to Image).': 'Kubesphere 支持從鏡像倉庫拉取鏡像以及通過代碼構建新的鏡像並部署',
  ...
  'Mount the configmap or secret to the specified directory.': '將配置文件或密鑰掛載至指定目錄',
  'Mount Volume': '掛載儲存卷',
  'Mount Volumes': '掛載儲存',
  mounted: '已掛載',
  ...
  'Please specify the read and write mode and mount path': '請指定讀寫方式及掛載路徑',
  ...
  'Pod replicas will be deployed on different nodes as much as possible.': '容器組副本將會盡量分散在不同的節點中',
  'Pod replicas will be deployed on the same node as much as possible.': '容器組副本將會盡量部署在同一節點上',
  ...
  'Support EmptyDir and PersistentVolumeClaim.': '支持臨時儲存卷以及持久化儲存卷',
  ...
  'The number of failed jobs allowed to be retained.': '允許保留的失敗的任務個數',
  'The number of successful jobs allowed to be retained.': '允許保留的成功的任務個數',
  'Timeout(s)': '超時時間(秒)',
  'UI Mode': '界面模式',
  ...
  Workloads: '工作負載',
  'You can allow Pod replicas to run on specified nodes.': '可以讓容器組副本在指定的節點運行',
  ...
  JOB_DESC: '任務 (Job) 負責批量處理短暫的一次性任務，即僅執行一次的任務，它保證批處理任務的一個或多個容器組成功結束。',
  JOB_CREATE_DESC: '任務 (Job) 負責批量處理短暫的一次性任務，即僅執行一次的任務，它保證批處理任務的一個或多個容器組成功結束。',
  CRONJOB_DESC: '定時任務 (CronJob) 管理基於時間的任務，例如在給定時間點只運行一次，或週期性地在給定時間點運行。',
  CRONJOB_CREATE_DESC: '定時任務 (CronJob) 管理基於時間的任務，例如在給定時間點只運行一次，或週期性地在給定時間點運行。',
  ...
  MAX_DAEMON_UNAVAILABLE_POD_DESC: '升級過程中「允許存在的不可用的容器組」所占總容器組數的最大百分比或數量',
  MAX_DEPLOY_UNAVAILABLE_POD_DESC: 'The maximum number of Pods that can be unavailable during the update.',
  MAX_SURGE_POD_DESC: '升級過程中「允許超出副本數量的容器組」的最大數量或百分比',
  ROLLING_UPDATE_POD_TIP: '更新時，會根據目前容器組的副本數對最小可用及最大數量進行限制；最小容器組數不可以超過目前副本數，且最大容器組數量不能超過目前副本數的2倍。',
  ONDELETE_ALERT_TIP: '控制器不會自動更新容器組，將會在容器組被手動刪除時，更新並替換容器組實例。',
  ROLLINGUPDATE_ALERT_TIP: '滾動升級將逐步用新版本的實例替換版本的實例，升級的過程中，業務流量會同時負載平衡分布到新舊的實例上，因此業務不會中斷。',
  RECREATE_ALERT_TIP: '替換升級會先刪除舊的容器組，再創建新容器組；升級過程中業務會中斷。',
  ...
  PORT_NAME_DESC: '端口名最長 63 個字元，只能包含小寫字母、數字及分隔符號("-")，且必須以小寫字母開頭, 字母或數字結尾',
  WORKLOAD_PORT_NAME_DESC: '端口名最長 15 個字元，只能包含小寫字母、數字及分隔符號("-")，且必須以小寫字母開頭, 字母或數字結尾',
  ...
  JOB_BACK_OFF_LIMIT_DESC: '失敗嘗試次數，若失敗次數超過該值，則任務不會繼續嘗試工作',
  ...
  CRONJOB_CRON_DESC: '按照給定的時間計畫運行工作。語法參照 <a href="//en.wikipedia.org/wiki/Cron" target="_blank">CRON</a>。Kubernetes 預設使用 UTC 時間, 請注意根據時區調整定時計畫。',
  START_DEADLINE_SECONDS_DESC: '即在指定 啟動時間 + 啟動 Job 的期限 這個週期之内都可以啟動任務',
  VOLUME_EMPTY_TIP: 'No created volumes, please',
  HOST_PATH_DESC: 'HostPath 允許掛載主機上的文件系統到容器組裡面去。如果容器組需要使用主機上的文件，可以使用 HostPath。',
  SELECT_SECRET_DESC: '選擇要使用的密鑰以及將公開每個密鑰的文件路徑，文件路徑相當於裝載路徑，每個文件的内容都是密鑰的值。',
  ...
  CPU_REQUEST_TARGET_DESC: '當 CPU 使用率超過或低於此目標值時，將添加或刪除副本',
  MEMORY_REQUEST_TARGET_DESC: '當記憶體使用量超過或低於此目標值時，將添加或刪除副本',
  ...
  TIMEOUT_DESC: '等待探針完成多長時間。如果超過時間，則認為探測失敗。預設為1秒。最小值為1。',
  ...
  HPA_MSG: '根據 CPU 和記憶體使用情況自動伸縮副本。如果同時指定 CPU 和記憶體，則滿足任一條件後即添加或刪除副本',
  ...
  POD_CREATE_DESC: '容器組 (Pod) 是 Kubernetes 應用程式的基本執行單元，是您創建或部署的 Kubernetes 對象模型中最小和最簡單的單元。',
  ...
  COLLECT_SAVED_LOG_DESC: '您已開啟落盤紀錄收集，請至少添加一個儲存卷並指定紀錄所在目錄',
  PROJECT_COLLECT_SAVED_DISABLED_DESC: '請聯繫項目管理員在 “項目設置” => "高級設置" 中開啟項目的落盤紀錄收集功能',
  COLLECT_FILE_LOG_TIP: '當您添加儲存卷後(需要讀&寫模式)，您可以對儲存卷内的紀錄資訊進行收集。開啟後，會在容器組中以 SideCar 的方式注入 filebeat 容器鏡像，來收集相關紀錄。',
  ISTIO_PROTOCOL_TIP: '為了充分利用應用治理的能力，請選擇服務實際使用的協定。例如，如果服務暴露的是 HTTP 服務，則選擇 http 協定，會生成形如 http-[name] 的端口名稱。',
  ...
  DELETE_WORKLOAD_DESC: '您即將刪除工作負載 {resource}，請您進行確認是否刪除關聯資源?',
  CHOOSE_EXIST_VOLUME_DESC: '請選擇一個已經創建好的持久化儲存卷掛載至容器',
  REDEPLOY_CONFIRM_DESC: '您即將重新部署工作負載 {resource} ({type}) , 容器組將根據更新策略進行重新部署，您的業務可能會被暫時中斷。',
  CONTAINER_SECURITY_CTX_DESC: 'Security Context的目的是限制不可信容器的行為，保護系統和其他容器不受其影響。',
  POD_SECURITY_CONTEXT_DESC: '容器組 Security Context 可以為容器組内的容器提供預設的用戶和用戶組設置以及 seLinuxOptions 的參數設置，如果容器中已經對這些參數進行了定義，則優先以容器中的設置為準。',
  ACCESS_CONTROL_PRIVILEGED: '以特權模式運行(Privileged)',
  ACCESS_CONTROL_PRIVILEGED_DESC: '此時容器中的進程本質上等價於宿主節點上的 root 用戶。',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION: '允許擴大特權(AllowPrivilegeEscalation)',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION_DESC: '進程是否可以獲取比父進程更多的特權。當以特權模式運行時，則為允許狀態。',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM: '文件系統root唯讀(ReadOnlyRootFilesystem)',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM_DESC: '該容器的文件系統根路徑是否唯讀。',
  ...
  WORKLOAD_CREATE_DESC: '工作負載 (Workload) 通常是訪問服務的實際載體, 也是對節點紀錄收集、監控等系統應用的實際運行載體，是對一組容器組 (Pod) 的抽象模型。',
  ...
  REVISION_DESC: '對工作負載的資源模板進行修改後會生成一個新的紀錄並重新調度 容器組（Pod）進行版本的疊代，預設保存10個最近的版本。您可以根據修改紀錄進行重新部署。',
  CLUSTER_CONTAINER_IMAGE_DIFF_DESC: '根據不同的需要在不同的集群中設置不同的容器',
  ...
  HOST_PATH_WARNING: 'HostPath 將主機的文件系統掛載到Pod中，它使一些應用程式能逃出對其做出的隔離限制，請謹慎使用。',
