...
module.exports = {
  ...
  'How do I invite other members to the current project?': '邀請其他成員到目前項目中?',
  ...
  MULTI_CLUSTER_PROJECT_DELETE_TIP: '刪除多集群項目同時也會刪除依賴於 Host 集群上的同名項目,</br>請輸入{type}名稱 <strong>{resource}</strong> 確保您已經了解操作所带來的風險。',
  ...
  'Resource Quota': '資源配額',
  'Resource Type': '資源類型',
  'Select a user of the workspace as the manager of the project.': '選擇企業空間的用戶作為管理員。',
  ...
  'You can limit the number of resources. Blank means no limit.': '您可以對資源的數量進行限制, 不填寫即不限制',
  PROJECTS_DESC: 'KubeSphere 中的項目對應的是 Kubernetes 的 namespace，是對一組資源和對象的抽象集合，常用來將系統内部的對象劃分為不同的項目組或用戶組。',
  PROJECT_CREATE_DESC: 'KubeSphere 中的項目對應的是 Kubernetes 的 namespace，是對一組資源和對象的抽象集合，常用來將系統内部的對象劃分為不同的項目組或用戶組。',
  ...
  PROJECT_INTERNET_ACCESS_DESC: '在創建應用路由之前，需要先啟用外網訪問入口，即網關。這一步是創建對應的應用路由控制器，用來負責將請求轉發到對應的後端服務。',
  ...
  HOW_TO_USE_QUOTA_A: '資源配額（Resource Quotas）是用來限制用戶資源用量的一種機制，可以對 CPU、記憶體、容器組數量等進行配額限制。',
  WHAT_IS_LIMIT_RANGE_Q: '什麼是容器資源預設請求?',
  WHAT_IS_LIMIT_RANGE_A: '容器資源預設請求 (LimitRange) 基於項目的資源管理，包括容器組和容器的保留資源、最大限額等。',
  WHAT_IS_INTERNET_GATEWAY: '什麼是外網訪問網關?',
  WHAT_IS_COLLECT_FILE_LOG_A: '容器所掛載的儲存卷中的紀錄路徑以 glob 方式給出，可在工作負載中配置紀錄路徑以收集這些紀錄。需要管理員預先開啟落盤紀錄收集。',
  HOW_TO_INVITE_MEMBER_Q: '如何邀請成員？',
  HOW_TO_INVITE_MEMBER_A: '項目管理員或者擁有成員邀請權限的用戶可以邀請目前企業空間内的成員加入項目',
  CLOSE_FILE_LOG_TIP: '落盤紀錄收集即將關閉。 關閉後，已開啟落盤紀錄收集的服務在容器組副本重啟前將繼續進行落盤紀錄的收集，重啟後，將不再收集。</br>如果需要再次收集，請開啟落盤紀錄收集，並重起容器組副本。',
  ...
  PROJECT_ADVANCED_SETTINGS_DESC: '對項目中的外網訪問網關以及服務治理和落盤紀錄收集等配置進行設置',
  ...
  MULTI_CLUSER_RESOURCE_TIP: '目前資源為多集群資源，資源將分布在不同集群中共同來構成多集群資源，您可以切換到不同集群查看資源在該集群中的設置。',
  MULTI_CLUSTER_PROJECT_CREATE_DESC: '您可以創建多集群項目，讓項目運行在多個集群中，為應用提供快速疊代開發的容器環境並實現高可用。'
};