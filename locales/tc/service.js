...
module.exports = {
  ...
  'No related resources found with current service(s)': '目前服務下沒有關聯的資源',
  'Node Port': '節點端口',
  'Node Port(s)': '節點端口',
  'Not Associate': '不關聯',
  ...
  'The maximum session sticky time is 10800s (3 hours).': '會話保持時間預設是 10800 秒(即 3 小時)',
  'Virtual IP': '虛擬 IP',
  SERVICE_EXTERNAL_NAME_DESC: '通過返回 CNAME 和它的值，可以將服務映射到 externalName 字段的内容',
  TOTAL_WORKLOAD: '共 {count} 個工作負載',
  SERVICE_SELECTOR_AFFECT_1: '',
  SERVICE_SELECTOR_AFFECT_2: '共影響到 {count} 個工作負載',
  SERVICE_NAME_DESC: '最長 63 個字元，只能包含小寫字母、數字及分隔符號("-")，且必須以小寫字母開頭, 字母或數字結尾',
  SERVICE_DESC: '服務 (Service) 是定義了一類容器組的邏輯集合和一個用於訪問它们的策略。',
  SERVICE_CREATE_DESC: '服務是定義了一類 Pod 的邏輯集合和一個用於訪問它们的策略。您可選擇服務的類型或者創建服務的方式。<br/>KubeSphere 支持無狀態服務和有狀態服務，並支持通過代碼或者成品構建服務。',
  ...
  VIRTUAL_IP_DESC: '以集群為服務生成的集群内唯一的 IP 為基礎，集群内部可以通過此 IP 來訪問服務。',
  HEADLESS_SELECTOR_TITLE: '集群内部通過服務的後端 Endpoint IP 直接訪問服務 Headless (selector)',
  HEADLESS_SELECTOR_DESC: '集群不為服務生成 IP，集群内部通過服務的後端 Endpoint IP 直接訪問服務。此類型適合後端異構的服務，比如需要區分主從的服務。',
  ...
  ACCESS_LOADBALANCER_TIP: '通過雲服務商提供的負載平衡器來訪問服務 (LoadBalancer)',
  SERVICE_NODE_PORT_DESC: '如果您目前的網路與集群節點在同一網路内，那麼您可以透通集群IP地址+節點端口號進行訪問，或者通過節點 IP+節點端口進行訪問',
  ...
  DELETE_SERVICE_DESC: '您即將刪除服務 {resource}，請您進行確認是否刪除關聯資源?',
  ...
  SPECIFY_WORKLOAD_DESC: '指定工作負載可以將工作負載所創建的容器組副本的 Label 作為預填充内容',
