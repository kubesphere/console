...
module.exports = {
  ...
  NETWORK_POLICY_DESC: '通過配置網路策略，允許在同個集群内時現網路的隔離，也就是可以在某些實例（Pod）之間架起防火牆。',
  ...
  NETWORK_POLICY_EMP_DESC: '由於企業空間的設置，目前項目允許集群内其他項目進行訪問。啟用項目網路隔離後，將禁止其他項目訪問目前項目，同時可以自定義需要放行的項目、服務、外部地址',
  NETWORK_POLICY_STATUS: '項目網路隔離',
  NETWORK_POLICY_R_DESC1: '可以設置對以下資源進行訪問(匹配下面任意策略)',
  NETWORK_POLICY_R_DESC2: '可以允許以下資源對目前項目進行訪問(匹配下面任意策略)',
  NETWORK_POLICY_R1_TITLE: '集群内部白名單',
  NETWORK_POLICY_R1_DESC: '允許集群内部服務訪問',
  NETWORK_POLICY_R1_DESC1: '選擇指定的項目或者服務作為白名單成員，允許這些資源訪問目前項目。',
  NETWORK_POLICY_R2_TITLE: '集群外部 IP 地址',
  NETWORK_POLICY_R2_DESC: '允許集群外部 CIRD 訪問',
  NETWORK_POLICY_R2_DESC1: '選擇特定的 IP CIDR 範圍以允許作為入口來源或出口目的地。',
  NETWORK_POLICY_D_DESC: '匹配 Egress(出口) 流量以及 Ingress(入口) 流量',
  NETWORK_POLICY_D_DESC2: 'CIDR 代表一個合法的 IP 地址區段,例如 “192.168.1.1/24”',
  NETWORK_POLICY_D_OP1: 'Egress(出口)',
  NETWORK_POLICY_D_OP2: 'Ingress(入口)',
  NETWORK_POLICY_CREATE_DESC: '通過配置網路策略控制同一集群内 Pod 之間的流量以及來自外部的流量，從而實現隔離應用並增強應用的安全性。',
  CIDR_DESC: '將根據流量的方向',
