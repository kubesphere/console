...
module.exports = {
  ...
  'The app has not received the request for a long time, please visit the app and try traffic management': '應用長時間未收到請求，請訪問應用後嘗試流量治理',
  ...
  APP_DEPLOYMENT_DESC: '輕量化、可移植、自包含的軟體封裝技術，使應用可以在幾乎任何地方以相同的方式運行。',
  ...
  APP_GOVERNANCE_DESC: '開啟應用治理後會在每個組件中以 SideCar 的方式注入 Istio-proxy 容器 <a href="https://istio.io/docs/setup/kubernetes/additional-setup/sidecar-injection/" target="_blank">了解更多</a>',
  ...
  APPLICATION_TYPE_DESC: 'KubeSphere 支持來自於應用商店和應用倉庫的應用部署(基於 Helm)，同樣也支持自制應用(Application CRD)。',
  HOW_TO_USE_APPLICATION_GOVE_Q: '如何使用應用治理？',
  HOW_TO_USE_APPLICATION_GOVE_A: '使用應用治理需要創建自制應用並對每項服務開啟服務治理功能',
  FROM_APP_STORE_DESC: '來自KubeSphere官方應用商店，提供高質量應用和簡易的部署方式',
  FROM_APP_TEMPLATES_DESC: '來自於企業空間的自制應用模板以及應用倉庫中添加的第三方 Helm 應用模板',
