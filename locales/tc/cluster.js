...
module.exports = {
  ...
  CLUSTER_WAITING_JOIN_DESC: '暫時沒有可用的節點，集群為不可以用狀態，您可以添加以下配置文件以啟用該集群',
  CLUSTER_AGENT_TIP_1: '請在通過 SSH 在目標集群中創建一個名稱為 agent.yaml 的文件',
  ...
  CLUSTER_AGENT_TIP_3: '通過命令行執行 <span class="code">kubectl create -f agent.yaml</span>',
  ...
  CLUSTER_VISIBILITY_A1: '集群可以通過“編輯可見範圍”將集群授權給不同的企業空間使用',
  CLUSTER_VISIBILITY_Q2: '什麼是公開集群?',
  CLUSTER_VISIBILITY_A2: '公開狀態的集群意味著平台内的用戶都可以使用該集群，並在集群中創建和調度資源',
  SELECT_HOST_CLUSTER_WARNING: '請盡量不要在 Host 集群上創建資源，以免 Host 集群負載過高，導致多集群穩定性下降。',
  HOST_CLUSTER_VISIBILITY_WARNING: '請謹慎將 Host 集群授權给企業空間，Host 集群負載過高會導致多集群穩定性下降。',
  CLUSTER_VISIBILITY_REMOVE_WARNING: '移除對企業空間的授權，將刪除該企業空間在當前擴展下的所有資源。',
  REMOVE_WORKSPACE_CONFIRM_TITLE: '確定移除授權？',
  REMOVE_WORKSPACE_CONFIRM_DESC: '確定移除對企業空間 {resource} 的授權？移除集群對企業空間的授權，將刪除該企業空間在目前集群下的所有資源！',
  ...
  CLUSTER_CREATING_TIP: '當前集群正在創建，暫時沒有可用的節點，所以集群為不可以用狀態',
  ...
  CLUSTER_UPGRADE_REQUIRED: '目前集群版本無法使用此功能，請升級到 {version} 或以上版本。',
  MEMBER_CLUSTER_UPGRADE_TIP: '低於 {version} 版本 member 集群無法使用此功能, 請將 member 集群升級到 {version} 或以上版本。'
};