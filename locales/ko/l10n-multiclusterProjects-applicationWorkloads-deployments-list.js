...
module.exports = {
  ...
  SET_AS_DEFAULT_REPOSITORY_DESC: '이미지 레지스트리를 기본 이미지 레지스트리로 설정합니다. 달리 지정하지 않은 경우 시스템은 기본 이미지 레지스트리의 이미지를 사용하여 응용 프로그램 워크로드를 만듭니다. 각 프로젝트에는 기본 이미지 레지스트리가 하나만 허용됩니다.',
  ...
  POD_GRACE_PERIOD_DESC: '파드가 종료될 때까지 유예 기간을 설정하여 유예 기간 이후 파드가 강제 종료 되도록 합니다.',
