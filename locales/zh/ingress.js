...
module.exports = {
  ...
  Mode: 'Mode',
  Paths: 'Paths',
  'Please add at least one routing rule.': 'Please add at least one routing rule.',
  ...
  ROUTE_DESC: 'A Route provides a way to aggregate Services, and you can expose the cluster\'s internal Services outside through an externally accessible IP address.',
  ROUTE_CREATE_DESC: 'A Route provides a way to aggregate Services, and you can expose the cluster\'s internal Services outside through an externally accessible IP address.',
  ...
  RULE_SETTING_MODE_AUTO: 'By configuring DNS access and changing the domain name to {$hostname} + {$gateway address} +nip.io, you can access the Service via {$hostname}.{$gateway address} .nip.io:{$NodePort}. <br/>Please ensure that you are in an enabling network environment to access the gateway address.',
  RULE_SETTING_MODE_SPECIFY: 'Make sure that the domain name you set can be resolved to the IP address of the access portal. <br/>If you are in a private cloud environment, modify the local host file and then access it via {$domain name}:{$node port}.',
  ...
  NO_INTERNET_ACCESS_TIP: 'The gateway address is not found in the current project, so you cannot use Auto Generate. Please contact your project administrator to turn it on in <strong>Internet Access</strong>.',
