...
module.exports = {
  ...
  'No related resources found with current service(s)': '当前服务下没有关联的资源',
  'Node Port': '节点端口',
  'Node Port(s)': '节点端口',
  'Not Associate': '不关联',
  ...
  'The maximum session sticky time is 10800s (3 hours).': '会话保持时间默认是 10800 秒(即 3 小时)',
  'Virtual IP': '虚拟 IP',
  SERVICE_EXTERNAL_NAME_DESC: '通过返回 CNAME 和它的值，可以将服务映射到 externalName 字段的内容',
  TOTAL_WORKLOAD: '共 {count} 个工作负载',
  SERVICE_SELECTOR_AFFECT_1: '',
  SERVICE_SELECTOR_AFFECT_2: '共影响到 {count} 个工作负载',
  SERVICE_NAME_DESC: '最长 63 个字符，只能包含小写字母、数字及分隔符("-")，且必须以小写字母开头, 字母或数字结尾',
  SERVICE_DESC: '服务 (Service) 是定义了一类容器组的逻辑集合和一个用于访问它们的策略。',
  SERVICE_CREATE_DESC: '服务是定义了一类 Pod 的逻辑集合和一个用于访问它们的策略。您可选择服务的类型或者创建服务的方式。<br/>KubeSphere 支持无状态服务和有状态服务，并支持通过代码或者制品构建服务。',
  ...
  VIRTUAL_IP_DESC: '以集群为服务生成的集群内唯一的 IP 为基础，集群内部可以通过此 IP 来访问服务。',
  HEADLESS_SELECTOR_TITLE: '集群内部通过服务的后端 Endpoint IP 直接访问服务 Headless (selector)',
  HEADLESS_SELECTOR_DESC: '集群不为服务生成 IP，集群内部通过服务的后端 Endpoint IP 直接访问服务。此类型适合后端异构的服务，比如需要区分主从的服务。',
  ...
  ACCESS_LOADBALANCER_TIP: '通过云服务商提供的负载均衡器来访问服务 (LoadBalancer)',
  SERVICE_NODE_PORT_DESC: '如果您当前的网络与集群节点在同一网络内，那么您可以通集群IP地址+节点端口号进行访问，或者通过节点 IP+节点端口进行访问',
  SERVICE_TYPE: '您可以自定义创建 无状态服务 或者 有状态服务',
  SERVICE_TYPES_Q: '服务的类型',
  SERVICE_TYPES_A: '服务分为无状态服务 (Virtual Service + Depolyment) 及有状态服务 (Headless Service +Statefulset), 无状态服务中多个副本可以共享一个存储卷、有状态服务需要拥有自己独立的存储卷',
  SCENARIOS_FOR_SERVICES_Q: '无状态服务和有状态服务的使用场景?',
  SCENARIOS_FOR_SERVICES_A: '无状态服务适用于不需要数据持久化的场景，并且多个实例对统一请求的响应式相同的场景(例如 Nginx、Tomcat 等)；有状态服务适用于需要数据存储功能的服务、或者指多线程类型的服务，队列等 (mysql 数据库、kafka、zookeeper 等)。',
  SERVICE_SIMPLE_DESC: 'Create a service from an existing pods group',
  DELETE_SERVICE_DESC: '您即将删除服务 {resource}，请您进行确认是否删除关联资源?',
  SERVICE_FROM_CODE: '通过代码构建新的服务',
  SERVICE_FROM_ARTIFACTS: '通过制品构建新的服务',
  SERVICE_FROM_CODE_DESC: '您可以将已有的代码通过  Source to Image 的方式构建成镜像并部署',
  ...
  STATEFUL_SERVICE_DESC: '有状态服务用来管理有状态应用，可以保证部署和扩容缩容的顺序，提供了稳定的持久化存储和网络标识，有序伸缩等',
  STATELESS_SERVICE_DESC: '容器服务中最常用的一种服务，通过定义容器组模板来控制容器组状态，包括滚动升级和回滚',
  SERVISE_SIMPLE_DESC: '通过已有的容器组来创建服务',
  SERVICE_PORTS_DESC: '设置容器镜像暴露的端口以及服务端口',
  SPECIFY_WORKLOAD_DESC: '指定工作负载可以将工作负载所创建的容器组副本的 Label 作为预填充内容',
