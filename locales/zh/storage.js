...
module.exports = {
  ...
  VOLUME_DESC: 'A volume is used for workloads created by users. It represents a resource object for the persistent storage of workloads.',
  ...
  VOLUME_CREATE_DESC: 'A volume is used for workloads created by users. It represents a resource object for the persistent storage of workloads.',
  ...
  VOLUME_STORAGE_CLASS_DESC: 'The cluster administrator configures the storage server parameters and provides storage for the user by type.',
  ...
  STORAGE_CLASS_DESC: 'A StorageClass provides a way for administrators to configure the "classes" of storage they offer.',
  ...
  STORAGE_CLASS_SET_DEFAULT_DESC: 'After the default storage class is set, the system will create volumes of this class by default if no special requirement is added. Only one default storage class is allowed in a Kubernetes cluster.',
  ...
  WHAT_IS_STORAGE_CLASS_A: 'The Storage Class is configured by the cluster administrator to configure storage server parameters and provide storage for cluster users by class.',
  WHAT_IS_LOCAL_VOLUME_Q: 'What is a Local Volume?',
  WHAT_IS_LOCAL_VOLUME_A: 'A local volume is a mounted local storage device, such as a disk, partition, or directory.',
  CHOOSE_STORAGE_SYSTEM_TIP: 'Choose the Storage System You Need',
  PROVISIONER_DEPENDENCE_DESC: 'You need to deploy the corresponding storage plugin for a storage system first so that it can provide services.',
  ...
  QINGCLOUD_CSI_DESC: 'The QingCloud CSI plugin implements the Container Storage Interface (CSI) of Kubernetes, enabling the container orchestrator (CO) to use QingCloud storage. Currently, the QingCloud CSI plugin supports block storage services on QingCloud Platform. For detailed information, refer to <a href="https://github.com/yunify/qingcloud-csi/blob/master/README.md#feature-matrix">features</a>.',
  QINGCLOUD_CSI_TYPE_DESC: 'On QingCloud Public Cloud Platform, 0 represents high performance volume. 2 represents high capacity volume. 3 represents super high performance volume. 5 represents Enterprise Server SAN. 100 represents standard volume.',
  ...
  GLUSTERFS_RESTURL_DESC: 'Heketi service url which provisions gluster volumes on demand. ',
  GLUSTERFS_ID_DESC: 'Gluster ID',
  GLUSTERFS_RESTAUTHENABLED_DESC: 'Enable authentication to the REST server.',
  GLUSTERFS_RESTUSER_DESC: 'The Gluster REST service/Heketi user who has access to creating volumes in the Gluster Trusted Pool.',
  GLUSTERFS_SECRET_NAMESPACE_DESC: 'Identification of Secret instance that contains user password to use when talking to Gluster REST service.',
  GLUSTERFS_SECRET_NAME_DESC: 'These parameters are optional; an empty password will be used when both secretNamespace and secretName are omitted.',
  GLUSTERFS_GID_MIN_DESC: 'The minimum value of GID range for the storage class. A unique value (GID) in this range (gidMin-gidMax) will be used for dynamically provisioned volumes. These are optional values.',
  GLUSTERFS_GID_MAX_DESC: 'The maximum value of GID range for the storage class. A unique value (GID) in this range (gidMin-gidMax) will be used for dynamically provisioned volumes. These are optional values.',
  GLUSTERFS_VOLUME_TYPE_DESC: 'The volume type and its parameters can be configured with this optional value.',
  QINGCLOUD_VOLUME_TAGS_DESC: 'Tags will be associated automatically when a hard drive is created. Please separate multiple tags with commas.',
  CEPHRBD_MONITORS_DESC: 'Ceph monitors, comma delimited. This parameter is required.',
  CEPHRBD_ADMIN_ID_DESC: 'Ceph client ID that is capable of creating images in the pool.',
  CEPHRBD_ADMIN_SECRET_NAME_DESC: 'The provided secret must have type “kubernetes.io/rbd”.',
  ...
  CEPHRBD_IMAGE_FEATURES_DESC: 'This parameter is optional and should only be used if you set imageFormat to “2”.',
  DEPENDENT_STORAGE_CLASS_DELETE_TIPS: 'Please check if the storage class is being used by other resources. If there is any resource dependent on the storage class, you need to disable the resource first.',
  ...
  VOLUMESNAPSHOT_DESC: 'A volume snapshot refers to a snapshot of a volume at a particular time point. It can be used to create new volumes (preset the snapshot data) or restore an existing volume to its previous status (displayed in snapshots).',
  VOLUMESNAPSHOT_CREATE_DESC: 'A volume snapshot refers to a snapshot of a volume at a particular time point. It can be used to create new volumes (preset the snapshot data) or restore an existing volume to its previous status (displayed in snapshots).',
  ...
  VOLUME_MONITORING_TIP: 'Kubernetes collects volume usage data, excluding data from unmounted volumes. For path-based volumes such as OpenEBS/Local PV and NFS, the data collected may be different from the actual amount. For detailed information, see <a href="https://github.com/kubesphere/kubesphere/issues/2921" target="_blank">volume monitoring data analysis</a>.'
};