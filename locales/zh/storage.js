...
module.exports = {
  ...
  'Storage System': '存储系统',
  STORAGE_CLASS_CREATE_DESC: '存储类型 (StorageClass) 是由集群管理员配置存储服务端参数，并按类型提供存储给集群用户使用。',
  StorageClasses: '存储类型',
  storageclasses: '存储类型',
  ...
  VOLUME_CREATE_DESC: '存储卷供用户创建的工作负载使用，是将工作负载数据持久化的一种资源对象。',
  VOLUME_SETTINGS_DESC: '按需填写存储卷的容量大小，存储卷大小和访问模式必须与存储类型和存储服务端能力相适应，访问模式通常选择为 RWO。',
  VOLUME_STORAGE_CLASS_DESC: '由集群管理员配置存储服务端参数，并按类型提供存储给用户使用。',
  ...
  STORAGE_CLASS_DESC: '存储类型 (StorageClass) 是由集群管理员配置存储服务端参数，并按类型提供存储给集群用户使用。',
  ...
  STORAGE_CLASS_SET_DEFAULT_DESC: '设置为默认存储类型后，如果没有特殊指定，系统将默认创建该类型的存储卷。一个 Kubernetes 集群中仅允许设置一个默认存储类型',
  ...
  WHAT_IS_STORAGE_CLASS_A: '存储卷类型 (StorageClass) 是由集群管理员配置存储服务端的参数，并按类型提供存储给集群用户使用。',
  WHAT_IS_LOCAL_VOLUME_Q: '什么是本地存储卷 (Local Volume) ?',
  WHAT_IS_LOCAL_VOLUME_A: '本地存储卷表示挂载的本地存储设备，如磁盘、分区或目录。',
  ...
  QINGCLOUD_CSI_DESC: 'QingCloud CSI 插件实现了 CSI 接口，并使容器编排平台能够使用 QingCloud 云平台的存储资源。实现了块存储插件，可以对接云平台块存储资源。<a href="https://github.com/yunify/qingcloud-csi/blob/master/README.md">详细信息</a>',
  QINGCLOUD_CSI_TYPE_DESC: '在青云云平台中，0 代表性能型硬盘，2 代表容量型硬盘，3 代表超高性能型硬盘，5 代表企业级分布式 SAN（NeonSAN）型硬盘，100 代表基础型硬盘，200 代表企业型硬盘',
  ...
  DEPENDENT_STORAGE_CLASS_DELETE_TIPS: '请确认是否有资源依赖该存储类型。若存在依赖，请先将依赖的资源关闭，以免影响资源功能',
  CREATE_VOLUME_BY_STORAGECLASS: '通过存储类型',
  CREATE_VOLUME_BY_SNAPSHOT: '通过存储卷快照创建',
  SELECT_SNAPSHOT_TO_CREATE_VOLUME: '选择已有的存储卷快照进行创建',
  VOLUMESNAPSHOT_DESC: '存储卷快照表示存储卷的时间点副本。快照可用于配置新卷（预先填充快照数据）或将现有存储卷还原到先前状态（由快照表示）',
  VOLUMESNAPSHOT_CREATE_DESC: '存储卷快照表示存储卷的时间点副本。快照可用于配置新卷（预先填充快照数据）或将现有存储卷还原到先前状态（由快照表示）',
  WHAT_IS_VOLUME_SNAPSHOTS: '什么是存储卷快照',
  CLUSTER_VOLUME_DIFF_DESC: '可针对不同集群，选择不同的存储类型',
  VOLUME_MONITORING_TIP: 'Kubernetes 采集的是存储卷的设备用量数据，未挂载的存储卷暂时采集不到，并且对于如 OpenEBS/Local PV、NFS 等路径型存储卷通常与实际用量有一定出入。详见<a href="https://github.com/kubesphere/kubesphere/issues/2921" target="_blank">存储卷监控数据分析</a>。'
};