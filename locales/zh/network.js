...
module.exports = {
  ...
  NETWORK_ISOLATION_DESC: 'By configuring network isolation to control traffic among Pods within the same cluster and traffic from outside, users can isolate applications with security enhanced.',
  NETWORK_POLICY_Q: 'How do I use a network policy better?',
  NETWORK_ISOLATION_Q: 'How do I use network isolation better?',
  NETWORK_POLICY_A: 'We have compiled several common application scenarios based on the actual scenarios, and you can refer to the documentation for more information.',
  ...
  NETWORK_POLICY_EMP_DESC: 'The current project can be accessed by other projects in the cluster based on the workspace settings. Once network isolation is enabled for the project, other projects will be unable to access it while you can still customize specific projects, services or external addresses that can be released.',
  NETWORK_POLICY_STATUS: 'Project Network Isolation',
  NETWORK_POLICY_R_DESC1: 'You can set access to the following resources (matching any of the following policies)',
  NETWORK_POLICY_R_DESC2: 'The following resources can be allowed to access the current project (matching any of the following policies)',
  NETWORK_POLICY_R1_TITLE: 'Cluster Internal Allowlist',
  NETWORK_POLICY_R1_DESC: 'Allow access for services within the cluster',
  NETWORK_POLICY_R1_DESC1: 'Select a specific project or service as an allowlist member so that these resources can access the current project.',
  NETWORK_POLICY_R2_TITLE: 'Cluster External IP Address',
  NETWORK_POLICY_R2_DESC: 'Allow access for CIRD outside the cluster',
  NETWORK_POLICY_R2_DESC1: 'Select a specific IP CIDR range as an entry source or exit destination.',
  NETWORK_POLICY_D_DESC: 'Match Egress traffic and Ingress traffic',
  NETWORK_POLICY_D_DESC2: 'CIDR is a string representing a valid IP block, such as "192.168.1.1/24".',
  NETWORK_POLICY_D_OP1: 'Egress',
  NETWORK_POLICY_D_OP2: 'Ingress',
  NETWORK_POLICY_CREATE_DESC: 'The network policy is configured to allow network isolation within the same cluster, that is, the ability to build a firewall between certain instances (pods).',
