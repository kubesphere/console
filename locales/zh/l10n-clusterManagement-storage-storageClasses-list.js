...
module.exports = {
  ...
  STORAGE_CLASS_DESC: '存储类型（StorageClass）支持动态卷供应，使管理员能够按需创建新的存储卷。',
  // List
  ...
  QINGCLOUD_VOLUME_TAGS_DESC: '为存储卷添加标签。使用半角逗号（,）分隔多个标签。',
  // List > Create > > GlusterFS > Storage Class Settings
  GLUSTERFS_RESTURL_DESC: '供应存储卷的 Heketi REST URL，例如，<Heketi 服务集群 IP 地址>:<Heketi 服务端口号>。',
  ...
  GLUSTERFS_VOLUME_TYPE_DESC: '存储卷的类型。该值可为 none，replicate:<副本数>，或 disperse:<数据>:<冗余数>。如果未设置该值，则默认存储卷类型为 replicate:3。',
  RESTURL: 'REST URL',
  CLUSTER_ID: '集群 ID',
