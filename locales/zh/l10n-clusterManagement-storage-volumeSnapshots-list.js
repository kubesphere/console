...
module.exports = {
  ...
  WHAT_IS_VOLUME_SNAPSHOT_CLASS_A: '存储卷快照类型可用于指定存储卷快照的属性。从存储系统的相同存储卷实例创建的快照可能有不同的属性，因此这些属性无法通过相同存储类型生成的存储卷来表示。',
  WHAT_IS_VOLUME_SNAPSHOT_CONTENT_Q: '什么是存储卷快照内容？',
  WHAT_IS_VOLUME_SNAPSHOT_CONTENT_A: '存储卷快照内容是从集群管理员分配的存储卷创建的快照。与存储卷实例类似，存储卷快照也是集群中的资源。',
