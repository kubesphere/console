...
module.exports = {
  'A/B Testing': 'A/B 测试',
  'Allocate all traffic proportionally to grayscale release components': '将所有的流量按比例分配给灰度组件',
  ...
  'Introduce traffic that meets the following rules into grayscale version': '将符合以下规则的流量引入灰度版本',
  'Job offline': '任务下线',
  ...
  'Policy Config': '策略配置',
  POLICY_REQUEST_CONTENT_TIP: '端口协议非 HTTP, HTTP2 或 gRPC, 不能发布按内容分配的策略',
  'Prefix Match': '前缀匹配',
  ...
  traffic: '流量',
  'Traffic comes from the following operating systems': '流量来自于以下操作系统',
  ...
  BLUE_GREEN_DEPLOYMENT_DESC: '蓝绿发布提供了一种零宕机的部署方式，在<strong>保留旧版本的同时部署新版本</strong>。两个版本中总有一个版本处于在线状态接收所有流量，另一个版本则保持待机，如果有问题可以快速处理。',
  CANARY_RELEASES_DESC: '将一部分实际流量引入一个新版本进行测试，测试新版本的性能和表现，在保证系统整体稳定运行的前提下，尽早发现新版本在实际环境上的问题。',
  TRAFFIC_MIRROR_DESC: '流量镜像用来更为真实地测试新版本，提前发现问题，同时不对生产环境产生影响，从而，提高版本发布的安全性可靠性。',
  ...
  GRAY_RELEASE_DESC: '灰度发布是迭代的软件产品在生产环境安全上线的一种重要手段，提供软件版本部署升级平滑过渡的一种发布方式。',
  GRAYSCALE_RELEASE_DESC: '灰度发布是迭代的软件产品在生产环境安全上线的一种重要手段，提供软件版本部署升级平滑过渡的一种发布方式。',
  GRAY_RELEASE_BY_CONTENT_TIP: '基于请求内容发布：根据请求内容配置规则，只有请求内容中满足特定条件的流量会切分到灰度版本上。该策略只对直接访问的入口服务有效。',
  MIRROR_POLICY_DESC: '微服务让我们可以更快地实现交付。在追求快速的同时保证业务的稳定性。流量镜像可以降低变更所带来的风险，并且同时让生产环境上的新变化变得更为安全。</br>流量镜像将生产环境的流量复制灰度版本中，在真正负载实时流量（也就是客户流量）之前验证新版本是不是没有问题',
  RATIO_MODIFY_NOTIFY_CONTENT: '您已将{version}版本的目标流量配比调整至 {ratio}%  , 您也可以继续调整目标流量配比，或者使它立即生效。',
  CANARY_BY_TRAFFIC_DESC: '根据基于流量比例规则，对组件 {component} 的访问满足 {ratio}% 的请求流量会转发到灰度版本 {newVersion}',
  DEPLOY_APP_CONFIRM: '确认部署示例应用?',
  DEPLOY_APP_TIP: '您即将部署示例应用 {name}',
  DEPLOY_SAMPLE_NO_INTERNET_ACCESS_TIP: '当前项目中没有找到已开启应用治理的网关，因此您无法部署示例应用。请联系您的项目管理员在【高级设置】中设置外网访问方式',
  PREREQUEST_FOR_USE_GRAYRELEASE_Q: '使用灰度发布的前提条件?',
  PREREQUEST_FOR_USE_GRAYRELEASE_A: '使用灰度发布功能您需要创建自定义应用，并且将需要灰度发布的服务开启服务治理功能。',
  JOB_OFFLINE_WARNING: '为了保证服务的正常运行，任务下线需要将需要保留一个可用版本并且将流量全部切换至该版本。我们需要您选择一个需要下线的版本。系统会自动将全部流量切换至该另外的可用版本。',
