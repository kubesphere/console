...
module.exports = {
  ...
  'Additional metadata settings for resources such as Labels and Annotations.': '对资源进行额外的元数据设置，例如 Label 和 Annotation',
  ...
  'Container CPU Resource Request, 1 Core = 1000m': '容器的 CPU 资源请求值, 1核 = 1000m',
  ...
  'Job Settings': '任务设置',
  'Job Template': '任务模板',
  'KubeSphere supports pulling images from the Image Registries and creating new images through source code (Source to Image).': 'Kubesphere 支持从镜像仓库拉取镜像以及通过代码构建新的镜像并部署',
  ...
  'Mount the configmap or secret to the specified directory.': '将配置文件或密钥挂载至指定目录',
  'Mount Volume': '挂载存储卷',
  'Mount Volumes': '挂载存储',
  mounted: '已挂载',
  ...
  'Please specify the read and write mode and mount path': '请指定读写方式及挂载路径',
  ...
  'Pod replicas will be deployed according to the default policy.': '容器组副本将根据默认策略部署',
  ...
  'Support EmptyDir and PersistentVolumeClaim.': '支持临时存储卷以及持久化存储卷',
  ...
  WORKLOAD_REASON_PROGRESSDEADLINEEXCEEDED: '处理超时(ProgressDeadlineExceeded)',
  ...
  WORKLOAD_DESC: '工作负载 (Workload) 通常是访问服务的实际载体, 也是对节点日志收集、监控等系统应用的实际运行载体，是对一组容器组 (Pod) 的抽象模型。',
  ...
  JOB_DESC: '任务 (Job) 负责批量处理短暂的一次性任务，即仅执行一次的任务，它保证批处理任务的一个或多个容器组成功结束。',
  JOB_CREATE_DESC: '任务 (Job) 负责批量处理短暂的一次性任务，即仅执行一次的任务，它保证批处理任务的一个或多个容器组成功结束。',
  CRONJOB_DESC: '定时任务 (CronJob) 管理基于时间的任务，例如在给定时间点只运行一次，或周期性地在给定时间点运行。',
  CRONJOB_CREATE_DESC: '定时任务 (CronJob) 管理基于时间的任务，例如在给定时间点只运行一次，或周期性地在给定时间点运行。',
  ...
  REPLICAS_SCALE_NOTIFY_CONTENT: '您已将工作负载的副本数调整为 <strong>{num}</strong>, 您也可以继续调整副本数量，或者您可以使它立即生效。',
  ...
  MAX_DAEMON_UNAVAILABLE_POD_DESC: '升级过程中「允许存在的不可用的容器组」所占总容器组数的最大百分比或数量',
  MAX_DEPLOY_UNAVAILABLE_POD_DESC: '升级过程中可能不可用的 Pod 的最大数量。',
  MAX_SURGE_POD_DESC: '升级过程中「允许超出副本数量的容器组」的最大数量或百分比',
  ROLLING_UPDATE_POD_TIP: '更新时，会根据当前容器组的副本数对最小可用及最大数量进行限制；最小容器组数不可以超过当前副本数，且最大容器组数量不能超过当前副本数的2倍。',
  ONDELETE_ALERT_TIP: '控制器不会自动更新容器组，将会在容器组被手动删除时，更新并替换容器组实例。',
  ROLLINGUPDATE_ALERT_TIP: '滚动升级将逐步用新版本的实例替换旧版本的实例，升级的过程中，业务流量会同时负载均衡分布到新老的实例上，因此业务不会中断。',
  RECREATE_ALERT_TIP: '替换升级会先删除旧的容器组，再创建新容器组；升级过程中业务会中断。',
  ...
  IMAGE_DESC: '要从私有镜像仓库部署，需要先<a href={link} target="_blank">创建镜像仓库密钥</a>，然后拉取镜像。',
  ...
  PORT_NAME_DESC: '端口名最长 63 个字符，只能包含小写字母、数字及分隔符("-")，且必须以小写字母或数字开头及结尾',
  WORKLOAD_PORT_NAME_DESC: '端口名最长 15 个字符，只能包含小写字母、数字及分隔符("-")，且必须以小写字母或数字开头及结尾',
  ...
  JOB_BACK_OFF_LIMIT_DESC: '失败尝试次数，若失败次数超过该值，则任务不会继续尝试工作',
  ...
  CRONJOB_CRON_DESC: '按照给定的时间计划运行工作。语法参照 <a href="//en.wikipedia.org/wiki/Cron" target="_blank">CRON</a>。Kubernetes 默认使用 UTC 时间, 请注意根据时区调整定时计划。',
  START_DEADLINE_SECONDS_DESC: '即在指定 启动时间 + 启动 Job 的期限 这个周期之内都可以启动任务',
  VOLUME_EMPTY_TIP: 'No created volumes, please',
  HOST_PATH_DESC: 'HostPath 允许挂载主机上的文件系统到容器组里面去。如果容器组需要使用主机上的文件，可以使用 HostPath。',
  SELECT_SECRET_DESC: '选择要使用的密钥以及将公开每个密钥的文件路径，文件路径相当于装载路径，每个文件的内容都是密钥的值。',
  ...
  CPU_REQUEST_TARGET_DESC: '当 CPU 使用率超过或低于此目标值时，将添加或删除副本',
  MEMORY_REQUEST_TARGET_DESC: '当内存使用量超过或低于此目标值时，将添加或删除副本',
  ...
  TIMEOUT_DESC: '等待探针完成多长时间。如果超过时间，则认为探测失败。默认为1秒。最小值为1。',
  PERIOD_SECONDS_DESC: '执行探测的频率（以秒为单位）。默认为10秒。最小值为1。',
  SUCCESS_THRESHOLD_DESC: '探测失败后，连续最小成功探测为成功。默认值为1。最小值为1。存活探针和启动探针内必须为1。',
  FAILURE_THRESHOLD_DESC: '探针进入失败状态时需要连续探测失败的最小次数。',
  HPA_MSG: '根据 CPU 和内存使用情况自动伸缩副本。如果同时指定 CPU 和内存，则满足任一条件后即添加或删除副本',
  ...
  MONITORING_ALERT_DESC: '当前监控最多可显示五个副本的运行状态监控，当超过五个副本时，可以单击具体监控项的「查看全部副本」，查看更多的副本监控。',
  ...
  IMAGE_PULL_POLICY_NEVER_DESC: '仅会使用本地镜像，如果本地不存在所需镜像，则会导致容器异常',
  ...
  POD_ASSIGNED_DESC: '根据容器组中容器设置的请求值 (即 Request) 作为容器调度时资源分配的判断依据。只有节点上可分配总量 ≥ 容器请求值时，才允许将容器调度到该节点。',
  POD_DESC: '容器组 (Pod) 是 Kubernetes 应用程序的基本执行单元，是您创建或部署的 Kubernetes 对象模型中最小和最简单的单元。',
  POD_CREATE_DESC: '容器组 (Pod) 是 Kubernetes 应用程序的基本执行单元，是您创建或部署的 Kubernetes 对象模型中最小和最简单的单元。',
  ...
  COLLECT_SAVED_LOG_DESC: '您已开启落盘日志收集，请至少添加一个存储卷并指定日志所在目录',
  PROJECT_COLLECT_SAVED_DISABLED_DESC: '请联系项目管理员在 “项目设置” => "高级设置" 中开启项目的落盘日志收集功能',
  COLLECT_FILE_LOG_TIP: '当您添加存储卷后(需要读&写模式)，您可以对存储卷内的日志信息进行收集。开启后，会在容器组中以 SideCar 的方式注入 filebeat 容器镜像，来收集相关日志。',
  ISTIO_PROTOCOL_TIP: '为了充分利用应用治理的能力，请选择服务实际使用的协议。例如，如果服务暴露的是 HTTP 服务，则选择 http 协议，会生成形如 http-[name] 的端口名称。',
  CONTAINER_LOG_PATH_TIP: '容器日志相对路径是从容器挂载路径开始的路径，可以 glob 方式给出，多组时以英文逗号分隔。例如当容器挂载路径为 /data 时，容器日志相对路径配置为 log/*.log，表示匹配 /data/log 目录下所有 .log 后缀文件。<br/>若需要匹配 /data/log 目录及其子目录下的所有 .log 后缀文件，可将容器日志相对路径配置为 log/**/*.log',
  ADD_SUBPATH_TIP: '仅适用于存储卷挂载，不适用于主机路径映射',
  DELETE_WORKLOAD_DESC: '您即将删除工作负载 {resource}，请您进行确认是否删除关联资源?',
  CHOOSE_EXIST_VOLUME_DESC: '请选择一个已经创建好的持久化存储卷挂载至容器',
  REDEPLOY_CONFIRM_DESC: '您即将重新部署工作负载 {resource} ({type}) , 容器组将根据更新策略进行重新部署，您的业务可能会被暂时中断。',
  CONTAINER_SECURITY_CTX_DESC: 'Security Context的目的是限制不可信容器的行为，保护系统和其他容器不受其影响。',
  POD_SECURITY_CONTEXT_DESC: '容器组 Security Context 可以为容器组内的容器提供默认的用户和用户组设置以及 seLinuxOptions 的参数设置，如果容器中已经对这些参数进行了定义，则优先以容器中的设置为准。',
  ACCESS_CONTROL_PRIVILEGED: '以特权模式运行(Privileged)',
  ACCESS_CONTROL_PRIVILEGED_DESC: '此时容器中的进程本质上等价于宿主节点上的 root 用户。',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION: '允许扩大特权(AllowPrivilegeEscalation)',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION_DESC: '进程是否可以获取比父进程更多的特权。当以特权模式运行时，则为允许状态。',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM: '文件系统root只读(ReadOnlyRootFilesystem)',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM_DESC: '该容器的文件系统根路径是否为只读。',
  RUN_AS_NON_ROOT: '仅允许非 Root 用户',
  RUN_AS_NON_ROOT_DESC: 'Kubernetes 在运行容器之前将执行检查，以确保容器进程不是以 root 用户（UID为0）运行，否则将不能启动容器。',
  RUN_AS_USER_DESC: '执行容器 entrypoint 进程的 UID。默认为 docker 引擎的 GID',
  RUN_AS_USER_GROUP_DESC: '执行容器 entrypoint 进程的 GID。默认为 docker 引擎的 GID',
  WORKLOAD_CREATE_DESC: '工作负载 (Workload) 通常是访问服务的实际载体, 也是对节点日志收集、监控等系统应用的实际运行载体，是对一组容器组 (Pod) 的抽象模型。',
  CONTAINER_RESOURCE_LIMIT_TIP: '请设置容器的资源限制与资源预留，这将能够帮助系统更好地调度容器，提高稳定性。您也可在【项目设置】中，通过【基本信息】->【项目管理】->【编辑资源默认请求】，来统一设置默认值。',
  COMPARE_WITH: '与上一个版本 {version} 的对比',
  REVISION_DESC: '对工作负载的资源模板进行修改后会生成一个新的记录并重新调度 容器组（Pod）进行版本的迭代，默认保存10个最近的版本。您可以根据修改记录进行重新部署。',
  CLUSTER_CONTAINER_IMAGE_DIFF_DESC: '根据不同的需要在不同的集群中设置不同的容器',
  ...
  HOST_PATH_WARNING: 'HostPath 将主机的文件系统挂载到Pod中，它使一些应用程序能逃出对其做出的隔离限制，请谨慎使用。',
