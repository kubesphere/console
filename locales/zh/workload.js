...
module.exports = {
  ...
  'Horizontal Pod Autoscaling has been set': 'Horizontal Pod Autoscaling has been set',
  'Host Path': 'Host Path',
  'Host Port': 'Host Port',
  ...
  'KubeSphere supports pulling images from the Image Registries and creating new images through source code (Source to Image).': 'KubeSphere supports pulling images from the Image Registries and creating new images through source code (Source to Image).',
  ...
  'Mount the configmap or secret to the specified directory.': 'Mount the ConfigMap or Secret to the specified directory.',
  ...
  'Please select at least one container to mount': 'Please select at least one container to mount',
  ...
  'Please specify the read and write mode and mount path': 'Please specify the read and write mode and mount path',
  ...
  'Pod replicas will be deployed according to the default policy.': 'Pod replicas will be deployed according to the default policy.',
  ...
  'Support EmptyDir and PersistentVolumeClaim.': 'Support EmptyDir and PersistentVolumeClaim.',
  ...
  Workloads: 'Workloads',
  'You can allow Pod replicas to run on specified nodes.': 'You can allow Pod replicas to run on specified nodes.',
  WORKLOAD_DESC: 'A workload is usually the actual carrier for accessing a Service, and is also the actual running carrier for system applications such as node log collection and monitoring. A workload is an abstract model for a group of Pods.',
  ...
  STATEFULSET_CREATE_DESC: 'StatefulSet is used to manage stateful applications, manages the deployment and scaling of a set of Pods, and provides guarantees about the ordering and uniqueness of these Pods.',
  ...
  DAEMONSET_CREATE_DESC: 'A DaemonSet ensures that all (or some) Nodes run a copy of a Pod. Typically, a DaemonSet is used to running a logs collection, monitoring daemon or other system management applications.',
  JOB_DESC: 'A Job is used to execute one-time tasks. A Job creates one or more Pods and ensures that a specified number of them successfully terminate.',
  JOB_CREATE_DESC: 'A Job is used to execute one-time tasks. A Job creates one or more Pods and ensures that a specified number of them successfully terminate.',
  CRONJOB_DESC: 'A CronJob creates Jobs on a time-based schedule. For example, it runs a Job periodically on a given schedule or only runs a Job once on a given time.',
  CRONJOB_CREATE_DESC: 'A CronJob creates Jobs on a time-based schedule. For example, it runs a Job periodically on a given schedule or only runs a Job once on a given time.',
  CRONJOB_NAME_DESC: 'It can only contain lowercase letters, numbers and hyphens("-"), and must begin and end with a lowercase letter or a number. The value can contain a maximum of 52 characters.',
  ...
  REPLICAS_SCALE_NOTIFY_CONTENT: 'You are going to change the number of replicas of your workload to {num}. You can continue to change it, or you can make the change take effect immediately.',
  ...
  MAX_DAEMON_UNAVAILABLE_POD_DESC: 'The maximum number of DaemonSet pods that can be unavailable during the update.',
  MAX_DEPLOY_UNAVAILABLE_POD_DESC: 'The maximum number of Pods that can be unavailable during the update.',
  MAX_SURGE_POD_DESC: 'The maximum number of Pods that can be scheduled above the desired number of Pods.',
  ...
  ONDELETE_ALERT_TIP: 'The controller will not automatically update the Pod. It will update and replace Pod instances when the Pod is manually deleted.',
  ROLLINGUPDATE_ALERT_TIP: 'A rolling update means the instance of the old version will be gradually replaced with new ones. During the upgrade process, the traffic will be load balanced and distributed to the old and new instances simultaneously, so the service will not be interrupted.',
  RECREATE_ALERT_TIP: 'All existing Pods will be killed before new ones are created. Please note that the service will be interrupted during the update process.',
  ...
  MIN_READY_SECONDS_DESC: 'Specifies the minimum number of seconds before a newly created Pod of DaemonSet is treated as available',
  IMAGE_PLACEHOLDER: 'Select the registry from the drop-down list or enter a public registry address',
  IMAGE_REGISTRY_PLACEHOLDER: 'Select the registry secret',
  IMAGE_DESC: 'To deploy from a private image repository, you need to <a href={link} target="_blank">create an image registry secret</a> first and then pull the image.',
  ...
  PORT_INPUT_DESC: 'The name is required and cannot be duplicated when there are multiple ports,',
  PORT_NAME_DESC: 'The port name can only contain lowercase letters, numbers and hyphens("-"), and must begin and end with a lowercase letter or a number. The value can contain a maximum of 63 characters.',
  WORKLOAD_PORT_NAME_DESC: 'The port name can only contain lowercase letters, numbers and hyphens("-"), and must begin and end with a lowercase letter or a number. The value can contain a maximum of 15 characters.',
  ...
  JOB_ACTIVE_DL_SECONDS: 'It specifies the duration in seconds relative to the startTime that the Job may be active before the system tries to terminate it; value must be positive integer.',
  ...
  CRONJOB_CRON_DESC: 'It runs a job periodically on a given time-based schedule. Please see <a href="//en.wikipedia.org/wiki/Cron" target="_blank">CRON</a> for grammar reference. By default, Kubernetes adopts UTC. Please adjust scheduled plans based on the time zone.',
  START_DEADLINE_SECONDS_DESC: 'An optional deadline in seconds for starting the job if it misses its scheduled time for any reason. ',
  ...
  SELECT_SECRET_DESC: 'Select the key you want to use and the file path where each key will be exposed. The file path is equivalent to the mount path. The content of each file is the value of the key.',
  ...
  CPU_REQUEST_TARGET_DESC: 'Replicas will be increased when CPU usage exceeds this target value, on the contrary it will be decreased.',
  MEMORY_REQUEST_TARGET_DESC: 'Replicas will be increased when memory usage exceeds this target value, on the contrary it will be decreased.',
  ...
  ADD_VOLUME_TEMPLATE_DESC: 'The life cycle of the volume will be the same as that of the Pod.',
  EDIT: 'Edit',
  MORE: 'More',
  VIEW_YAML: 'View YAML',
  ...
  INITIAL_DELAY_DESC: 'Number of seconds after the container has started before liveness probes are initiated.',
  TIMEOUT_DESC: 'Number of seconds after which the probe times out. It defaults to 1 second and the minimum value is 1.',
  PERIOD_SECONDS_DESC: 'Probe frequency (in seconds), which defaults to 10 seconds. The minimum value is 1.',
  SUCCESS_THRESHOLD_DESC: 'Minimum consecutive successes for the probe to be considered successful after having failed. It defaults to 1 and must be 1 for liveness and startup. The minimum value is 1.',
  FAILURE_THRESHOLD_DESC: 'Minimum consecutive failures for the probe to be considered failed after having succeeded. It defaults to 3 and the minimum value is 1.',
  HPA_MSG: 'Automatically autoscale the replicas according to CPU and memory usage. If both CPU and memory are specified, the replicas is added or deleted after any of the conditions are met.',
  ...
  MONITORING_ALERT_DESC: 'The current monitoring graph displays five replicas at most. You can click "View All Replicas" to view more monitoring graphs if the number of replicas exceeds five.',
  ...
  IMAGE_PULL_POLICY_IFNOTPRESENT_DESC: 'Pull the image only if it does not exist locally.',
  IMAGE_PULL_POLICY_NEVER_DESC: 'Only the local image will be used, which will cause the container to be abnormal if the required image does not exist locally.',
  LIVENESS_PROBE_DESC: 'This check method is used to detect if the container is alive.',
  READINESS_PROBE_DESC: 'This check method is used to detect if the container is ready to service requests.',
  STARTUP_PROBE_DESC: 'This check method is used to detect if the container is started successfully.',
  ...
  POD_CONDITION_INITIALIZED_DESC: 'All init containers have started successfully.',
  POD_CONDITION_READY: 'Ready',
  POD_CONDITION_READY_DESC: 'The pod is already running and can be accessed through the service.',
  ...
  POD_CONDITION_PODSCHEDULED_DESC: 'The pod has been successfully assigned to a node.',
  POD_ASSIGNED_DESC: 'The request value (ie, Request) set by the pod in the pod group is used as the basis for determining the resource allocation. Only when the amount that can be allocated in the node ≥ the pod\'s requst value, can the pod be allocated to this node.',
  ...
  POD_CREATE_DESC: 'A Pod is the basic execution unit of a Kubernetes application, representing the smallest and simplest unit in the Kubernetes object model that you create or deploy.',
  ...
  COLLECT_SAVED_LOG_DESC: 'You have enabled the option of collecting the logs on disk. Please add at least one volume and specify the directory in which the log is located.',
  PROJECT_COLLECT_SAVED_DISABLED_DESC: 'Please contact the project admin to enable Disk Log Collection in "Project Settings => Advanced Settings".',
  COLLECT_FILE_LOG_TIP: 'When you add the PersistentVolumeClaim（PVC) (read & write mode), you can collect log information from the PVC. Once it is enabled, the filebeat pod image, which can be used to collect relevant logs, will be installed through SideCar.',
  ISTIO_PROTOCOL_TIP: 'In order to fully use the Application Governance function, please select the protocol the service will actually be using. For example, please select HTTP protocol for a HTTP service and you will get a port name like http-[name].',
  CONTAINER_LOG_PATH_TIP: 'The container log relative path is the path from the pod mount path, which can be given in glob mode. When there are multiple groups, separate them by commas. For example, when the pod mount path is /data, the pod log relative path is configured as log/*.log, which means that all .log suffix files in the /data/log directory are matched. If you need to match all .log suffix files in the /data/log directory and its subdirectories, you can configure the pod log relative path to log/**/*.log.',
  ADD_SUBPATH_TIP: 'It\'s only appropriate for mounting volumes but not for hostpath',
  DELETE_WORKLOAD_DESC: 'You are about to delete the workload(s) {resource}. Please confirm whether to delete the associated resource?',
  CHOOSE_EXIST_VOLUME_DESC: 'Select a volume that has been created and mount it to the container.',
  REDEPLOY_CONFIRM_DESC: 'You are about to redeploy the workload {resource} ({type}). The pod will be redeployed according to the update strategy, and your business may be temporarily interrupted.',
  CONTAINER_SECURITY_CTX_DESC: 'A security context defines privilege and access control settings for a Pod or Container.',
  POD_SECURITY_CONTEXT_DESC: 'Pod Security Context can provide default user and user group settings and seLinuxOptions parameter settings for containers in the Pod. If these parameters have been defined in the container, the settings in the container take precedence.',
  ACCESS_CONTROL_PRIVILEGED: 'Privileged',
  ACCESS_CONTROL_PRIVILEGED_DESC: 'The process in the container is essentially equivalent to the root user on the host node.',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION: 'AllowPrivilegeEscalation',
  ACCESS_CONTROL_ALLOWPRIVILEGEESCALATION_DESC: 'Whether the process can acquire more privileges than the parent process. When running in privileged mode, it is enabled.',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM: 'ReadOnlyRootFilesystem',
  ACCESS_CONTROL_READONLYROOTFILESYSTEM_DESC: 'Whether the container\'s file system root path is read-only.',
  RUN_AS_NON_ROOT: 'Run as non root',
  RUN_AS_NON_ROOT_DESC: 'Kubernetes will perform a check before running the container to ensure that the container process is not running as the root user (UID 0), otherwise the container will not start.',
  RUN_AS_USER_DESC: 'The UID to run the entrypoint of the container process. Defaults to user specified in image metadata if unspecified.',
  RUN_AS_USER_GROUP_DESC: 'The GID to run the entrypoint of the container process. Uses runtime default if unset.',
  WORKLOAD_CREATE_DESC: 'A workload is usually the actual carrier for accessing a Service, and is also the actual running carrier for system applications such as node log collection and monitoring. A workload is an abstract model for a group of Pods.',
  CONTAINER_RESOURCE_LIMIT_TIP: 'Please set the container\'s resource limits and requests, which will help the system to better schedule and improve stability. In Project Settings, you can also set the default value (Basic Information -> Manage Project -> Edit Resource Default Request).',
  ...
  CLUSTER_ENV_DIFF_DESC: 'Different container enviroments can be set in different clusters',
  POD_SCALE_DESC: 'The number of Pod instances that can be scaled',
  REPLICAS_AVAILABLE: 'Available',
  REPLICAS_EXPECTED: 'Expected',
  SYNC_HOST_TIMEZONE_DESC: 'The time zone of the container will be consistent with that of the host after synchronization.',
  HOST_PATH_WARNING: 'A HostPath volume mounts a file or directory from the host node’s filesystem into your Pod. This is not something that most Pods will need, but it offers a powerful escape hatch for some applications.',
  ...
  IGNORE_CERT_DESC: 'A certificate error was found, do you want to ignore the certificate verification',
