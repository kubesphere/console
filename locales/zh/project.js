...
module.exports = {
  ...
  'Disk Log Collection of the project is about to be disabled.': '项目的落盘日志收集即将关闭.',
  'Edit Gateway': '编辑网关',
  'Edit Project': '编辑项目',
  'Edit Project Quota': '编辑项目配额',
  'Edit Resource Default Request': '编辑资源默认请求',
  'Empty value means no limit, CPU 1 Core = 1000m': '值为空表示无限制, CPU 1核 = 1000m',
  'Enter DevOps Project': '进入工程',
  'Enter Project': '进入项目',
  FED_HOST_NAMESPACE_TIP: '该项目为多集群项目的相关资源, 请勿在此项目下操作资源',
  'Gateway Info': '网关信息',
  'Gateway Not Set': '网关未设置',
  'Help Information': '帮助信息',
  'How do I invite other members to the current project?': '邀请其他成员到当前项目中?',
  ...
  MULTI_CLUSTER_PROJECT_DELETE_TIP: '删除多集群项目同时也会删除依赖于 Host 集群上的同名项目,</br>请输入{type}名称 <strong>{resource}</strong> 确保您已了解操作所带来的风险。',
  ...
  'Resource Quota': '资源配额',
  'Resource Type': '资源类型',
  'Select a user of the workspace as the manager of the project.': '选择企业空间的用户作为管理员。',
  ...
  'You can limit the number of resources. Blank means no limit.': '您可以对资源的数量进行限制, 不填即不限制',
  PROJECTS_DESC: 'KubeSphere 中的项目对应的是 Kubernetes 的 namespace，是对一组资源和对象的抽象集合，常用来将系统内部的对象划分为不同的项目组或用户组。',
  PROJECT_CREATE_DESC: 'KubeSphere 中的项目对应的是 Kubernetes 的 namespace，是对一组资源和对象的抽象集合，常用来将系统内部的对象划分为不同的项目组或用户组。',
  ...
  PROJECT_INTERNET_ACCESS_DESC: '在创建应用路由之前，需要先启用外网访问入口，即网关。这一步是创建对应的应用路由控制器，用来负责将请求转发到对应的后端服务。',
  ...
  HOW_TO_USE_QUOTA_A: '资源配额 (ResourceQuota) 是用来限制用户资源用量的一种机制，可以对 CPU、内存、容器组数量等进行配额限制。',
  WHAT_IS_LIMIT_RANGE_Q: '什么是容器资源默认请求?',
  WHAT_IS_LIMIT_RANGE_A: '容器资源默认请求 (LimitRange) 基于项目的资源管理，包括容器组和容器的保留资源、最大限额等。',
  WHAT_IS_INTERNET_GATEWAY: '什么是外网访问网关?',
  WHAT_IS_COLLECT_FILE_LOG_A: '容器所挂载的存储卷中的日志路径以 glob 方式给出，可在工作负载中配置日志路径以收集这些日志。需要管理员预先开启落盘日志收集。',
  HOW_TO_INVITE_MEMBER_Q: '如何邀请成员？',
  HOW_TO_INVITE_MEMBER_A: '项目管理员或者拥有成员邀请权限的用户可以邀请当前企业空间内的成员加入项目',
  CLOSE_FILE_LOG_TIP: '落盘日志收集即将关闭。 关闭后，已开启落盘日志收集的服务在容器组副本重启前将继续进行落盘日志的收集，重启后，将不再收集。</br>如果需要再次收集，请开启落盘日志收集，并重起容器组副本。',
  ...
  PROJECT_ADVANCED_SETTINGS_DESC: '对项目中的外网访问网关以及服务治理和落盘日志收集等配置进行设置',
  ...
  PROJECT_CLUSTER_SETTINGS_DESC: '选择要创建项目的集群. 当选择了多个集群时, 将创建多集群项目, 并会在 Host 集群上创建同名项目',
  ...
  MULTI_CLUSER_PROJECT_TIP: '当前项目为多集群项目，项目将分布在不同集群中共同来构成多集群项目，您可以切换到不同集群查看项目在该集群中的设置。',
  MULTI_CLUSER_RESOURCE_TIP: '当前资源为多集群资源，资源将分布在不同集群中共同来构成多集群资源，您可以切换到不同集群查看资源在该集群中的设置。',
  MULTI_CLUSTER_PROJECT_CREATE_DESC: '您可以创建多集群项目，让项目运行在多个集群中，为应用提供快速迭代开发的容器环境并实现高可用。'
};