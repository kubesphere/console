...
module.exports = {
  ...
  ALERT_MESSAGE_DESC: '告警消息记录了在工作负载级别的告警策略中，所有已发出的满足告警规则的告警信息。',
  ...
  REQUESTS_FOR_TRIGGER_AN_ALARM_A: '需要对资源设置告警策略，当资源某项指标达到了告警策略的阈值后即会触发并推送消息。',
  REQUESTS_FOR_PUSH_AN_ALARM_Q: '告警策略消息推送的前提条件?',
  REQUESTS_FOR_PUSH_AN_ALARM_A: '平台管理员需要选择告警通知的方式并配置对应的服务器。',
  HOW_TO_SUPRESS_AN_ALARM_Q: '如何对告警消息进行抑制?',
  HOW_TO_SUPRESS_AN_ALARM_A: '可以对每条告警策略进行多级别的设置，每个级别对应不同的告警周期及重复周期',
  ...
  ALERT_RULE_EXPRESSION_DESC: '告警规则可以通过 PromQL 语句来自定义，PromQL 相关语法请参考 <a href="https://prometheus.io/docs/prometheus/latest/querying/basics/" target="_blank" rel="noreferrer noopener">Prometheus Querying</a>'
};