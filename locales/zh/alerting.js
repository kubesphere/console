...
module.exports = {
  ...
  ALERT_POLICY_DESC: 'Specify conditions for alerting policies and customize monitoring targets and notification messages.',
  ALERT_MESSAGE_DESC: 'Alerting messages are generated based on the monitoring metrics and the workload alert policies in the current project. They can help users detect problems and respond in time.',
  ALERTING_POLICY_CREATE_DESC: 'You can find abnormal conditions of resources instantly by setting alerting policies.',
  ...
  REQUESTS_FOR_TRIGGER_AN_ALARM_A: 'You need to set an alerting policy for a resource. When a certain indicator of a resource reaches the threshold of the alerting policy, the message is triggered and pushed.',
  REQUESTS_FOR_PUSH_AN_ALARM_Q: 'How do I receive alerting messages if an alerting policy is triggered?',
  REQUESTS_FOR_PUSH_AN_ALARM_A: 'The platform administrator must select a notification method and configure the corresponding server.',
  HOW_TO_SUPRESS_AN_ALARM_Q: 'How do I suppress alerting messages?',
  HOW_TO_SUPRESS_AN_ALARM_A: 'Each alerting policy can be set at different levels, and each level corresponds to a different alert period and repetition period.',
  ALERTING_DURATION: 'Once the monitoring target meets the alert condition for the alert duration, the alert will be triggered.',
  ...
  ALERT_RULE_EXPRESSION_DESC: 'Alerting rules can be customized through PromQL statements. For PromQL related syntax, refer to <a href="https://prometheus.io/docs/prometheus/latest/querying/basics/" target="_blank" rel="noreferrer noopener"> Prometheus Querying</a>.'
};