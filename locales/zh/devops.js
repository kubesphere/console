...
module.exports = {
  ...
  DEVOPS_DESCRIPTION: 'DevOps 是一个独立的命名空间，其中定义了一组流水线；用户可以按照自己的方式对流水线进行分组 (例如：项目类型、组织类型)。',
  ...
  NO_RELATE_DEVOPS_DESC: '您可以创建或者联系项目管理员将您邀请到 DevOps 工程中开始您的工作',
  DEVOPS_PROJECT_CREDENTIALS_DESC: '凭证是包含了一些敏感数据的对象，如用户名密码，SSH 密钥和 Token 等, 用于在 Pipeline 运行时, 为拉取代码、push/pull 镜像、SSH 执行脚本等过程提供认证',
  DEVOPS_PROJECT_ROLES_DESC: '成员角色定义了在当前 DevOps 工程下用户所拥有的权限。',
  DEVOPS_PROJECT_MEM_DESC: '对工程内的成员进行管理及角色分配',
  DELETE_DEVOPS_TIP: '确定删除 DevOps 工程 <strong>{resource}</strong> ? 删除后将无法恢复, 工程下的资源也同时会被销毁。',
  DEVOPS_TIP_GITOPS_Q: '如何落地 GitOps？',
  DEVOPS_TIP_GITOPS_A: '创建关联代码仓库的流水线，并积极使用部署 Kubernetes 部署步骤，就可以实现 GitOps 了。',
