...
module.exports = {
  ...
  CLUSTER_TAG: 'Tag',
  CLUSTER_TAG_DESC: 'Indicate what the cluster is used for, such as a production environment, testing environment or demo environment.',
  ...
  CLUSTER_WAITING_JOIN_DESC: 'There are currently no nodes available. The cluster is unusable. You can add the following configuration file to import the cluster.',
  CLUSTER_AGENT_TIP_1: 'Please create a file named agent.yaml in the target cluster via SSH',
  CLUSTER_AGENT_TIP_1_DESC: 'For example <span class="code">vi agent.yaml</span>.',
  CLUSTER_AGENT_TIP_2: 'Copy the following configuration file to agent.yaml',
  CLUSTER_AGENT_TIP_2_DESC: 'The agent file can connect the target cluster to the platform.',
  CLUSTER_AGENT_TIP_3: 'Execute in command line <span class="code">kubectl create -f agent.yaml</span>',
  CLUSTER_AGENT_TIP_3_DESC: 'After you execute the command, wait for the update of the cluster status.',
  CLUSTER_CONDITIONS: 'Cluster Conditions',
  CLUSTER_BASE_INFO_DESC: 'This module summarizes the basic information of the current cluster.',
  ...
  UNBIND_CLUSTER_DESC: 'After the cluster is unbound, KubeSphere will be unable to manage the cluster and Kubernetes resources within the cluster will not be deleted.',
  ...
  CLUSTER_VISIBILITY_A1: 'You can assign the cluster to different workspaces by clicking Edit Visibility.',
  CLUSTER_VISIBILITY_Q2: 'What is a public cluster?',
  CLUSTER_VISIBILITY_A2: 'A public cluster means all platform users can access the cluster, in which they are able to create and schedule resources.',
  SELECT_HOST_CLUSTER_WARNING: 'Please try not to create resources on the host cluster to avoid excessive loads, which can lead to a decrease in the stability across clusters.',
  HOST_CLUSTER_VISIBILITY_WARNING: 'Please be careful to assign the host cluster to workspaces. If the load of the host cluster is too high, the stability across clusters will decrease.',
  CLUSTER_VISIBILITY_REMOVE_WARNING: 'Once the authorization for the workspace to use the cluster is removed, all resources of the workspace in the current cluster will be deleted.',
  REMOVE_WORKSPACE_CONFIRM_TITLE: 'Are you sure to remove authorization?',
  REMOVE_WORKSPACE_CONFIRM_DESC: 'Are you sure to remove the authorization to the workspace(s) {resource}? Once the authorization for the workspace to use the cluster is removed, all resources of the workspace in the current cluster will be deleted.',
  ...
  CLUSTER_NODE_INTERNAL_IP_DESC: 'The internal IP address for the connection between cluster nodes.',
  NODE_ROLE_DESC: 'The number of master nodes needs to be 1 or 3, and the number of woker nodes must be at least 1.',
  SSH_IP_ADDRESS_DESC: 'Please fill in the IP address that the current Host cluster can access.',
  ...
  CLUSTER_CREATING_TIP: 'The current cluster is being created, and there are no nodes available for the time being.',
  ...
  CLUSTER_CREATION_PROGRESS_TIP: 'It takes about 30 to 60 minutes to create the entire cluster depending on the cluster size and network.',
  CLUSTER_UPGRADE_REQUIRED: 'This function is not available in the current cluster version. Please upgrade to {version} or above.',
  MEMBER_CLUSTER_UPGRADE_TIP: 'Member Clusters of version lower than {version} cannot use this function. Please upgrade the Member Cluster to {version} or above.'
};